...
Imported.Quasi_ABS = 0.9;

//=============================================================================
 /*:
 * @plugindesc Action Battle Systen
 ...
 * =============================================================================
 *   For the Skill Key, set the input key that the skill is linked to. This will
 * depend on what input plugin you are using. For example, for Quasi Input
 * you will set it to a Quasi Key which is just the key with a # infront.
 *   You can also set the Skill Key to mouse1 for left click and mouse2 for
 * right click. mouse1 and mouse2 do not require any additional Input plugin
 * =============================================================================
 * ** Note tags
 * =============================================================================
 * Classes:
 *     <skillKeys>
 *     skillNumber: skillId canReassign?
 *     </skillKeys>
 *
 * Enemy Database:
 *     <respawn:X>
 ...
 *
 * =============================================================================
 * ** Setting up Skills, Part 1 Skill Settings
 * =============================================================================
 * Skill settings handles creating the skill global settings.
 *   Note tag:
 *       <absSettings>
 ...
 *       selecttarget: value
 *       </absSettings>
 *   collider:      shape  - box or circle
 *                  width  - the width of the box or diameter Y of circle
 *                  height - the height of the box or diameter X of circle.
 *   offset:        set to a distance to offset the collider.
 *                  Default: 0
 *   infront:       set to true or false.
 *                  when true, the collider will appear infront of the user.
 *                  when false, the collider will center with the user.
 *                  Default: false
 *    rotate:       set to true or false.
 *                  when true, the collider will rotate based on users direction.
 *                  when false, the collider will not rotate.
 *                  Default: false
 *    cooldown:     Set to a number in frames until you can use the skill again.
 *                  60 frames is 1 second.
 *                  Default: 0
 *    through:      Set to 0, 1, 2 or 3
 ...
 *                  Default: 0
 *    groundtarget: Set to the max distance for targeting.
 *                  If value is 0, groundtargeting will not be used.
 *                  Default: 0
 *    selecttarget: Set to the max distance for targeting.
 *                  If value is 0, selecttargeting will not be used.
 *                  Default: 0
 * * All distance values, are in pixel terms.
 * * Can not use groungtarget and selecttarget together.
 * * Settings with a default value are not needed unless you are changing
 *   it's value
 * =============================================================================
 * ** Setting up Skills, Part 2 Skill Sequence
 * =============================================================================
 *   Skill sequence run once the skill has been activated. Configuring the
 * sequence can let you make each skill look / run differently.
 *     Note tag:
 *         <absSequence>
 *         action
 *         </absSequence>
 ...
 * 		  user speed inc/dec
 *
 *      user move direction distance wait?
 *        Set direction to forward or backward
 *        Set distance to the number of pixels to move
 *        Set wait? to true or false, Default: false
 *        -  If true it will wait until the move is done before running next action
 *
 *      user jump direction distance wait?
 *        Set direction to forward or backward
 *        Set distance to the number of pixels to jump
 *        Set wait? to true or false, Default: false
 *        -  If true it will wait until the move is done before running next action
 *
 *      user teleport
 *        Teleports user to the skills location
 *
 *      user directionfix true/false
 *
 *      user pose posename wait?
 *        Set posename to the name of the pose
 *        Set wait? to true or false, Default: false
 *        -  If true it will wait until the move is done before running next action
 *        * Only works if you have QuasiSprite installed
 *
 *      user forceskill skillId
 *        Set skillId to the id of the skill you want to force
 *
 *    Actions for collider / skill:
 *      move direction distance duration wait?
 *        Set direction to forward or backwards
  *       Set distance to the number of pixels to move
  *       Set duration to the time to takes to complete the move
  *       Set wait? to true or false, Default: false
  *       -  If true it will wait until the move is done before running next action
  *
 *      trigger
 *        Starts the skill at current position, all skill need this to activate,
 *        unless you have the move action, which activates the skill once it
 *        hits a target.
 *
 *      wait frames
 *        Set frames to the time to wait
 *
 *      picture filename rotate? baseDirection
 *        Set filename to the name of the picture
 *        Set rotate? to true or false, Default: false
 *        - If set to true the picture will rotate with the skill
 *        Set baseDirection to the direction the image is facing, 2, 4, 6 or 8
 *
 *      trail filename rotate? baseDirection
 *
 *      collider show
 *
 *      collider hide
 *
 *      animation ID
 *        Set ID to the animation to play at this location
 *
 *    Other:
 ...
 *
 * =============================================================================
 * ** Setting up Skills, Part 3 Skill On Damage
 * =============================================================================
 *   Skill On damage will run before the skill damages it's target. Can be used
 * to move the target around when they get hit.
 *     Note tag:
 *         <absOnDamage>
 *         action
 *         </absOnDamage>
 *    Possible actions:
 *    Actions for target:
 *      target move direction distance
 *        Set direction to towards or away
 *        Set distance to the number of pixels to move
 *
 *      target jump direction distance
 *        Set direction to towards or away
 *        set distance to number of pixels to jump
 *
 ...
 *
 *      animation location
 *        Set location to 0 or 1
 *        - When 0 it will play at the skills location
 *        - When 1 it will play at the targets location
 * =============================================================================
 * Links
 *  - http://quasixi.com/mv/
...
var QuasiABS = (function() {
  ...
  QuasiABS.proccessParameters = function() {
    var parameters = PluginManager.parameters('QuasiABS');
    this.skillKey  = {};
  ...
  QuasiABS.Manager.startPopup = function(x, y, string, iconIndex, settings) {
    var popup = new Sprite_QuasiPopup();
    popup.setup(string, iconIndex, settings);
    popup.realX = x;
    popup.realY = y;
    popup.remove = true;
    this.addPicture(popup);
  };
  ...
  Skill_Sequencer.prototype.userJumpHere = function(action) {
    ...
    var x2 = x1 - this._character.cx();
    var y2 = y1 - this._character.cy();
    // -- TO DO
    // add a check to see if can jump to this
    // location
    this._character.pixelJump(x2, y2);
    this._waitForUserJump = action[4] ? action[4] === "true" : false;
  };
  ...
  Skill_Sequencer.prototype.startMove = function(action) {
    SceneManager._scene.addTempCollider(this._skill.collider, Number(action[3]));
    var radian = action[1] === "forward" ? this._skill.radian : action[1];
    radian = radian === "backward" ? this.reverseRadian(this._skill.radian) : radian;
    this.setSkillRadian(Number(radian));
    this.moveSkill(Number(action[2]), Number(action[3]));
    this._waitForMove = action[4] ? action[4] === "true" : false;
  };
  ...
  Game_Actor.prototype.displayLevelUp = function(newSkills) {
    QuasiABS.Manager.startPopup($gamePlayer.cx(), $gamePlayer.cy(), "Level Up!");
  ...
  Game_Event.prototype.onDeath = function() {
    ...
    if (exp > 0) {
      QuasiABS.Manager.startPopup($gamePlayer.cx(), $gamePlayer.cy(), "Exp: " + exp);
    }
    this.setupLoot();
    this.clearAgro();
    if (this._activeSkills.length > 0) {
  ...
  Game_Loot.prototype.collectDrops = function() {
    ...
    QuasiABS.Manager.startPopup(this.cx(), this.cy(), string, this._iconIndex);
    this.erase();
  ...
  Game_Loot.prototype.aoeCollect = function() {
    ...
    for (var name in display) {
      ...
      QuasiABS.Manager.startPopup(this.cx(), this.cy(), string, iconIndex);
    }
    if (totalGold > 0) {
      ...
      QuasiABS.Manager.startPopup(this.cx(), y, string, QuasiABS.goldIcon);
    }
  };
  ...
  Sprite_Character.prototype.updateDamagePopup = function() {
    ...
    if (this._damages.length > 0) {
      for (var i = 0; i < this._damages.length; i++) {
        ...
        this._damages[i].y = this.y - this._damages[i].startingH + 20;
      }
  ...
  Sprite_Character.prototype.setupDamagePopup = function() {
    ...
    if (this._battler._damageQueue.length > 0) {
      ...
      sprite.x = this.x;
      sprite.y = this.y - this.height + 20;
      sprite.startingH = this.height;
      var string;
      var settings = {};
      ...
      if (result.missed || result.evaded) {
        ...
        if (dmg >= 0) {
          ...
          string = Math.abs(dmg);
          settings.textColor = "#00ff00";
        }
      } else if (result.mpDamage) {
        string = String(result.mpDamage);
        settings.textColor = "#0000ff";
      }
      if (!string && string != 0) return;
      var iconIndex = result.damageIcon;
      if (result.critical) settings.textColor = "#FF8C00";
      sprite.setup(string, iconIndex, settings);
  ...
  Sprite_QuasiPopup.prototype.setup = function(string, iconIndex, settings) {
    ...
    settings = settings || {};
    this.bitmap.clear();
    this.bitmap.fontFace = settings.fontFace || "GameFont";
    this.bitmap.fontSize = settings.fontSize || 28;
    this.bitmap.textColor = settings.textColor || "#ffffff";
    this.bitmap.drawText(string, x, y, width, height, 'center');
    if (iconIndex) {
      var icon = new Sprite_Icon(iconIndex);
      var stringLength = this.bitmap.measureTextWidth(string);
      stringLength = Math.min(this.bitmap.width, stringLength);
      icon.anchor.y = 1;
      icon.anchor.x = 0.5;
      icon.x -= icon._iconW / 2 + stringLength / 2;
      this.addChild(icon);
    }
  };
  ...
  Sprite_QuasiPopup.prototype.update = function() {
    ...
    this.updatePosition();
    this.updateOpacity();
    if(this.remove && !this.isPlaying()) {
      QuasiABS.Manager.removePicture(this);
    }
  };

  Sprite_QuasiPopup.prototype.updatePosition = function() {
    this.dy--;
    this.y += this.dy;
    if (this.realY) this.realY--;
  };

  Sprite_QuasiPopup.prototype.updateOpacity = function() {
